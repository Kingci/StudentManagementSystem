...
<form id="form1" method="post" action="admin?action=insertCourse">
  ...
  <tr class='TABLE_TR_01'>
    ...
    <td >开课院系</td>
    <td><label for="collegeId"></label>
  		<select name="collegeId" id="collegeId">
  		<%
  		AdminDAO adminDAO = new AdminDAO();
  		Collection departmentColl = adminDAO.queryDepartment(null);
  		if(!departmentColl.isEmpty() && departmentColl != null){
			Iterator it = departmentColl.iterator();
			while(it.hasNext()){
				DepartmentForm departmentForm = (DepartmentForm)it.next();
				out.println("<option value=\""+ departmentForm.getCollegeId() +"\">"+ departmentForm.getCollegeName() +"</option>");
			}
		}
  		%>
  		</select></td>
